...
3. เอาไฟล์จากมอดนี้ไปแทนที่/เพิ่มในโปรเจกต์ที่เพิ่งสร้าง:
   - คัดลอกไฟล์
     `src/main/java/com/example/autototem/AutoTotemModClient.java`
     ไปวางที่ตำแหน่งเดียวกันในโปรเจกต์ใหม่ (แทนที่ไฟล์ตัวอย่างเดิมที่ generator สร้างมา เช่น `ExampleModClient.java` - ลบไฟล์เก่าทิ้งได้เลย)
...
├── README.md   (ไฟล์นี้)
└── src/main/
    ├── java/com/example/autototem/AutoTotemModClient.java   <- โค้ดหลักของมอด
    └── resources/
        ├── fabric.mod.json
